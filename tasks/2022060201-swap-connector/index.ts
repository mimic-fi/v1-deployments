...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  if (from && (force || !output.SwapConnector)) {
    for (const config of input.config.uniswapV3) await connector.connect(from).setUniswapV3Path(config.tokens, config.fees)
    for (const config of input.config.balancerV2) await connector.connect(from).setBalancerV2Path(config.tokens, config.poolIds)
