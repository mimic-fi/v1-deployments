...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  const factory = await task.deployAndVerify('BuyAndHoldStrategyFactory', [input.Vault], from, force, 'factory')

  const output = task.output()
  for (const strategy of input.strategies) {
    if (force || !output[strategy.name]) {
      const args = [strategy.token, strategy.hodlToken, strategy.slippage, strategy.metadata]
      const tx = await factory.create(...args)
      const event = await assertEvent(tx, 'StrategyCreated')
      const instance = await task.instanceAt('BuyAndHoldStrategy', event.args.strategy)
      logger.success(`Deployed strategy ${strategy.name} at ${instance.address}`)
      task.save({ [strategy.name]: instance.address })
      await task.verify('BuyAndHoldStrategy', instance.address, args)
    }
  }
}
